...
(function(){
// External-reader file open (issues #52 + #66 unified): the engine's
// native-path opener supports only mac/win/linux; on Android the page's
// file-mention buttons (title=path) and tool-row file links ([data-tool]
// edit/write/read) would otherwise surface "unsupported on android" through
// the engine RPC. When the shell exposes androidBridge.openNativePath,
// intercept clicks on file-path elements and route them to the external
// reader. On Android the engine-RPC fallback can never open anything, so the
// click is consumed unconditionally (fail-silent); relative paths are
// resolved by the engine host (/api/android/open-path, token-gated) against
// the live session cwds with an fs-exists disambiguation. Idempotent guard
// marker keeps the injected script safe against double registration.
if(window.__dshOpenInterceptInstalled)return;
window.__dshOpenInterceptInstalled=true;
...
// scripts — NOT edit/write-tool deliverables) keep the upstream open-blue
// style so openable paths look identical to file-mention buttons.
var STYLE_ID='dsh-openable-code-style';
(function(){
  if(document.getElementById(STYLE_ID))return;
  var st=document.createElement('style');
  st.id=STYLE_ID;
  st.textContent='.markdown code.dsh-openable,code.dsh-openable{color:var(--dsw-alias-state-business-primary);cursor:pointer;text-decoration:none}.markdown code.dsh-openable:hover,code.dsh-openable:hover{text-decoration:underline;text-underline-offset:3px}';
...
var scheduleScan=function(){
  ...
  scanTimer=setTimeout(function(){scanTimer=null;scanOpenable()},150);
};
var bootScan=function(){
  if(!document.body){setTimeout(bootScan,50);return}
  scanOpenable();
...
// Plain inline-code path tokens (images too): the upstream file-mention
// vocabulary only covers files produced by edit/write tools (deliverables
// locations); files created by bash (python PIL renders, curl downloads,
// ffmpeg frames — png/jpg/webp…) render as a plain <code> with NO open
// button. Treat a path-like <code> click as an open request (resolved via
// /api/android/open-path for relative paths; absolute pass through). The
// token is consumed only when it looks like a path — code stays inert.
document.addEventListener('click',function(e){
  var el=e.target;
  while(el&&el!==document.body&&!(el instanceof HTMLElement)){el=el.parentElement}
  if(!el||el===document.body)return;
  ...
  if(!code)return;
  // Skip when the code contains an interactive child (mention button/link)
  if(code.querySelector&&code.querySelector('button,a'))return;
